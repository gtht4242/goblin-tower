...
Power: {}
Status: {}
Description: {}""".format(self.floor, self.name, self.role, self.level, self.exp, round_up(self.exp, 5), self.health, self.max_health, self.power,
...
(or press ESC to return to class selection screen)""".format(player.stats()))
    while True:
        key = ord(getch())
        if key == 10:
            break
...
""".format(target.getx(board), target.gety(board)))
